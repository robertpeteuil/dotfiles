...
define_helpers() {
  # Time12a="\$(date +%-I:%M)"   # - eliminates leading 0 in hour
  Time12a="\$(date +%I:%M)"
  # TimeHour="\$(date +%-I)"
  # TimeMin="\$(date +%M)"
  PathShort="\w";
...
override_git_prompt_colors() {
  ...
  # RP - possible symbols - ⊙⊚⊚⊞⊓⊒⊑⊱⊳⋍⋍⨠
  # RP mod to change '+' symbol
  GIT_PROMPT_STAGED="${Red}⊙ "           # the number of staged files/directories
  # GIT_PROMPT_CONFLICTS="${Red}✖ "       # the number of files in conflict
  # RP - possible symbols - ×⊠⊗╳
  # RP mod to change 'X' symbol
  ...
  # RP - possible symbols - ★☆
  # GIT_PROMPT_CLEAN="${BoldGreen}☆ "      # a colored flag indicating a "clean" repo
  GIT_PROMPT_CLEAN="${BoldYellow}★ "      # a colored flag indicating a "clean" repo
  ...
  GIT_PROMPT_COMMAND_OK="${Blue}▸"    # indicator if the last command returned with an exit code of 0
  # GIT_PROMPT_COMMAND_FAIL="${Red}✘-_LAST_COMMAND_STATE_"    # indicator if the last command returned with an exit code of other than 0
  # GIT_PROMPT_COMMAND_FAIL="${Red}▸-_LAST_COMMAND_STATE_"    # indicator if the last command returned with an exit code of other than 0
  ...
  # GIT_PROMPT_START_USER="${Green}${HostElement}${ResetColor}:${Cyan}${PathShort}${ResetColor}"
  ...
  GIT_PROMPT_START_USER="_LAST_COMMAND_INDICATOR_${Green}${HostElement}${ResetColor}:${Cyan}${PathShort}${ResetColor}"
  # GIT_PROMPT_END_USER="${ResetColor}$ "
  GIT_PROMPT_END_USER=" \n${Cyan}${Time12a}${ResetColor} $ "
  # GIT_PROMPT_END_USER=" \n ${Yellow}${TimeHour}${ResetColor}:${Yellow}${TimeMin}${ResetColor} $ "
  # GIT_PROMPT_END_ROOT="${BoldRed}# "
  GIT_PROMPT_END_ROOT=" \n${Cyan}${Time12a}${BoldRed} # "
  # GIT_PROMPT_END_ROOT=" \n ${Yellow}${TimeHour}${ResetColor}:${Yellow}${TimeMin}${BoldRed} # "
}
